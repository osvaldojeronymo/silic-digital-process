export function IdentificacaoCard(data: any): HTMLElement {
  ...
  card.innerHTML = `
    ...
    <div class="bd">
      <div><strong>Protocolo da Contratação:</strong> <span>${data?.protocolo || 'N/A'}</span></div>
      <div><strong>Data do Protocolo da Contratação:</strong> <span>${data?.dataProtocolo || 'N/A'}</span></div>
